...
**No page numbers.** `counter(page)` and `@page` margin boxes are CSS Paged Media features Chrome does not implement. Do not attempt them, and do not simulate them with `position: fixed` — a fixed footer _is_ repeated per page in Chrome but overlaps body content on pages 2+ unless `@page { margin: 0 }` is paired with `box-decoration-break: clone`. That trade is not worth making here. See issue #376 Notes.
...
**The `soap` branch applies the empty-value fallback to all four sections, unconditionally.** That diverges from `formatNoteForClipboard`, which emits `soapNote.subjective` raw and applies its `canonicalValue` fallback only to the other three. The divergence is deliberate and is recorded here so it is not later read as an inconsistency and "corrected":

- **Raw is right for a clipboard string.** A `malaysian`-derived Subjective is composed by `toSoapNote` from five history fields that have each already fallen back, so a second pass adds nothing.
- **Raw is wrong on paper.** An empty section under a printed heading reads as a document that lost its content, rather than as a field the consultation never established. The distinction between unknown and absent is the one `docs/prd.md` §10 exists to protect, and on a printed record it has to survive without a reader to hover over.

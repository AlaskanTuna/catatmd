...
- **Recordings now come back with drafted, per-line speaker labels the doctor reviews.** The guesses are derived from segment timing and question-mark punctuation, never from the voices, and each line is a Doctor/Patient toggle the doctor can flip before an explicit apply, measured at 53% prefill accuracy on the reference recording (`docs/trd.md` §20.2). Requesting the timestamps that make this possible costs roughly half again the transcription time.

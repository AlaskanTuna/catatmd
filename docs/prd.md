...
- **First-use download cost**, and **no self-hosted fallback** — Render's free tier (512 MB, no GPU) cannot host Whisper. Self-hosted server-side ASR is also strictly dominated: audio would cross the network anyway, incurring the full privacy cost _plus_ compute cost, for worse accuracy than a hosted ASR provider. Where audio must leave the device, the path is therefore a hosted in-region provider under an explicit per-consultation consent act (`docs/trd.md` §20) — **built**, and never entered automatically. It stays inert in production until the provider key is set.

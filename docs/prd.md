...
| Diagnosis, triage decisions, treatment selection | Safety boundaries, not deferred features, see Section 10. **Recording** a diagnosis or a medication the doctor stated is in scope and transcription-bound; **producing** one is not (`docs/decisions.md` D-001)                                                                                                                                                                                                                                                                                                                                                                            |
...
- **Treatment is recorded, never selected.** Two fields carry medication, and neither is ever produced by the system. `medicationsDispensed` is populated only by extraction and must carry a verbatim transcript span. A `prescriptions` record is authored by the doctor dictating it and confirming it. No drug is suggested, no dose is validated, no interaction or allergy is checked, and a drug name the recogniser garbled yields a candidate the doctor accepts rather than a silent substitution. **Testable:** a fixture in which the doctor names no drug leaves both empty; a fixture containing a look-alike sound-alike pair (`amoxicillin` against `amoxapine`) must never persist a name the doctor did not say. Recorded as `docs/decisions.md` D-001.
...
| Transcription-bound medication capture | The same hinge applied to treatment. A dictated prescription is recorded and confirmed, never generated, dose-checked, or substituted (D-001) |

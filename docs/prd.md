...
| Input    | Bundled synthetic fixture · pasted transcript text · uploaded transcript file (`.txt` / `.json`) · live audio — transcribed **on the doctor's device by default**, with a hosted in-region alternative reachable only by an explicit per-consultation act (`docs/trd.md` §20) |
...
- **Consent quality is the weak point in that design, and it is unresolved.** The doctor on weak hardware is exactly the one offered the hosted path, at the moment of most friction — which is how meaningful consent degrades into clicking past an obstacle. Handled by keeping hosted findable but never funnelled; the wording and affordance are open (`docs/trd.md` §19 row 18).
...
- **Recordings now come back with drafted, per-line speaker labels the doctor reviews.** The guesses are derived from segment timing and sentence content (question direction, first-person symptom statements), never from the voices, and each line is a Doctor/Patient toggle the doctor can flip before an explicit apply. The first ruleset measured 53% prefill accuracy on the reference recording and was revised to sentence-level scoring (`docs/trd.md` §20.2); the revision is pinned by unit tests but not yet re-measured on audio. Requesting the timestamps that make this possible costs roughly half again the transcription time.

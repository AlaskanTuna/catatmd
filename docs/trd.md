...
**The ceiling is still reachable, and since #373 that no longer costs the doctor anything.** A long transcript can spend the whole window, and when it does the browser's `fetch` rejects while Render keeps working: nothing cancels it, and it persists before it answers. The review page therefore reads the record's `status` rather than waiting on this response, so a cut-off request now costs a poll interval rather than the analysis (§13, "The Analysis Lease").
...
**Status: `Built`:** implemented 13/08/26 (issue #8) as 11 hand-written chunks, then retired in favour of a single retrieval-only pipeline on 09/09/26 (`docs/decisions.md` D-002). Nothing is hand-written today; every citable document, the three originally curated ones included, is ingested through the same pipeline.

- The stable document references and the prompt serialisation live in `backend/src/guidelines/documents.ts` and `backend/src/guidelines/prompt.ts`; the retrieval algorithm lives in `backend/src/retrieval/`.
- The full CPG library, the three originally curated sources included, is indexed in `corpus/cpg/manifest.json` and stored in `guideline_document` and `guideline_chunk`.
...
### Two Citable Vocabularies, Not Two Tiers

There is one corpus now, not two. What differs is which artefact cites which shape of reference into it:
...
interface GuidelineChunk {
  ...
  documentId?: string // set on every retrieved chunk, joining it back to `GuidelineDocument`
  page?: number
  ocr?: boolean       // true when the span was OCRed from a scanned page
}
```
...
`sourceLicence` and `verbatimAllowed` were added 13/08/26 because the licensing difference between sources is legally load-bearing and the schema previously had no way to express it. `verbatimAllowed: false` means the chunk may be summarised and linked but never quoted; a `quote` present on such a chunk is a corpus-authoring defect and `GuidelineChunkSchema`'s refinement fails it at parse time before it reaches a route. The review UI (`frontend/src/review/SafetyCards.tsx`) renders such a chunk **attribution only**: title, publisher, year, page, a deep link to the source PDF at that page, and the licence line, with no summary and no quote span shown, matching the rendering a `doc:` reference gets, because neither carries a retrieved span the licence permits displaying.

### Source Selection — Resolved 13/08/26 (§19 Row 3, Closed)

Three sources anchor the corpus, and every other ingested CPG scopes around them:
...
`ClinicalSuggestionSchema.citations[].guidelineId` is `z.string()` in the shared schema (§3) — the shared package cannot depend on a backend-only corpus. The request-time schema used for the suggestions call (§12) narrows this field to `z.enum(corpusIds)`, where `corpusIds` is the live list of retrieved chunk ids at request time. A citation naming an id outside that set, a `doc:` reference included, fails `request.schema.safeParse()` inside `OpenAICompatibleClient.generate()` (§6, `Built`) and throws `LLMResponseError` — the suggestion never reaches the doctor. This is a schema-enforced rejection path, not a prompt instruction the model could choose to ignore.

**Resolved 13/08/26** — source selection and the redistribution stance are settled above, and `verbatimAllowed` now carries the distinction in the schema rather than in a comment. §19 row 3 is closed. **Resolved 09/09/26:** the MaHTAS residual is answered. A Malaysian CPG for this territory does exist: the Academy of Medicine library lists _Management of Sore Throat_ (2003, portal file id 284), 23 years old, now ingested alongside the NAG 2024 anchoring. One residual remains, still deliberately not represented as settled: no clinician has reviewed the summaries corrected by the 07/09/26 primary-source audit (`docs/prd.md` §12, issue #240).

### Retrieval Pipeline (Built 09/09/26)
...
| Condition                              | Behaviour                                                                                                                                                                                                                                            |
| -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Chunk table empty for the jurisdiction | Returns `[]` before any embedding call; no citable corpus is available, so `generateSuggestions()` forces zero suggestions (`outOfScope: true`) while red flags and gaps, which cite `doc:` references rather than the retrieved set, run unaffected |
| Embedding call fails                   | Logs `embedding_unavailable` and continues lexical-only                                                                                                                                                                                              |
| `retrieveGuidelines` throws            | The `/analyze` handler logs `retrieval_error` and proceeds with no citable corpus, the same forced-empty behaviour as above; analysis never fails on retrieval                                                                                       |
| Nothing ranks                          | Returns `[]`; `retrievedGuidelines` is omitted from the persisted analysis rather than stored empty, and the same forced-empty behaviour applies                                                                                                     |
...
Issue #250 also covers a narrower, still-open question: whether MOH-ARR spans (the NAG document) may be sent to the model for grounding at all, distinct from whether they may be displayed. The current posture, stated rather than assumed, is that they are sent, de-identified like every other retrieved span, and display is off: `verbatimAllowed: false` suppresses the summary and quote in the review UI (above) but does not remove the chunk from the model's candidate set. §19 row 23 tracks this as open.
...
No automatic retry inside `LLMClient` (§6, `Built`) — a failure on either call throws `LLMResponseError`, which the `/analyze` route (§13) catches and translates into a reverted `Consultation.status` plus an error response. That revert targets the status the record held before the press rather than the one this handler read, because a re-claimed stale row would otherwise be restored to `analyzing` (#373, §13). The doctor's only retry path is manually re-triggering analysis (`docs/prd.md` §8, step 5), matching CAP-5's "no autonomous action" constraint — nothing retries itself.
...
| `POST`  | `/api/consultations/:id/analyze` | session | —                                                                                                                                                                               | `200 { consultation }` / `409` / `500`                | allowed from `draft` or `awaiting_review` (re-analysis); `409` from `approved`, and from `analyzing` unless the claim has outlived `STALE_ANALYSIS_MS` (see "The Analysis Lease" below); the guard and the `analyzing` write are **one conditional `updateMany`**, so two concurrent presses cannot both proceed; on `LLMResponseError`, status reverts to its pre-call value, never to `analyzing`, and `500` returns `ErrorEnvelopeSchema`                                |
...
### The Analysis Lease, And Why The Client Polls (#373)

`analyzing` is a claim with an expiry, and the review page reads that claim rather than waiting on the response. Both halves come from the same fact: **the browser can stop listening before the API stops working.**

| Piece               | Where                                        | What it does                                                                                                                         |
| ------------------- | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `STALE_ANALYSIS_MS` | `shared/src/index.ts`, 5 minutes             | How long a claim is honoured. Shared, so the moment the client stops waiting is the moment the API starts accepting a retry          |
| `ANALYSIS_POLL_MS`  | `shared/src/index.ts`, 3 seconds             | How often `GET /api/consultations/:id` is re-read while the record says `analyzing`                                                  |
| The claim           | `backend/src/routes/consultations.ts`        | One `updateMany` whose `where` is the guard: `draft`/`awaiting_review`, or an `analyzing` row whose `updatedAt` is past the lease    |
| The poll            | `frontend/src/routes/ConsultationReview.tsx` | `refetchInterval` on the detail query, armed by the mutation's `onError` re-read and switched off when the status settles or expires |

**Why a lease.** The rollback runs in this process's `catch` only. A restart mid-run leaves `analyzing` written with nothing left to clear it, and the previous read-then-write guard then answered 409 to every retry from then on, permanently. Five minutes is chosen against the worst legitimate run: the retrieval branch is a 10s embedding call then a 90s chat call, sequential, around which sit de-identification, the rules engine, rehydration, two writes and two audit rows.

**Why `updatedAt` is the clock.** Nothing writes the consultation row between the claim and the persistence write, so while the status is `analyzing` it is when analysis started. This is a coupling, not a declaration: **adding a mid-run write means adding a real `analysisStartedAt` column**, not reading this one harder.

**Why the rollback target is derived.** A stale claim retaken by a new press arrives at the `catch` as `analyzing`, and restoring the status this handler read would wedge the row for another whole lease. The target is `awaiting_review` when an analysis is already attached and `draft` otherwise.

**What this does not change.** Analysis is still synchronous, still one JSON at the end, still inside the 120s ceiling of §6. Nothing retries autonomously: the poll re-reads a record, it never re-presses. Making the delay legible to the doctor rather than merely survivable is issue #224, and streaming `/analyze` (the `copilot` route proves the pattern works through the same rewrite) would remove the ceiling but not the reload, which is why polling came first.
...
The one edge the diagram does not show is `analyzing →(lease expires) analyzing`, a re-claim by a fresh press. It is not a fifth state: the row never leaves `analyzing`, and the transition is a new run replacing an abandoned one.
...
| `asr.live_session_minted`         | `POST /api/asr/live-sessions` issues a browser-usable key for ambient capture (#268, §20.10) | `{ clientReferenceId, model, region, maxSessionSeconds, consentAsserted }`. Identifiers and bounds, never content. Actor-only: no consultation exists at mint time. The reference id is what reconciles this row against the provider's usage log for an egress this server never observes. **Gains a closed `mode` when prescription dictation lands (#356)**, because the same route will then mint for two egress shapes with different session caps, and a row that cannot say which is a row that cannot be reconciled |
...
`GUIDELINE_CORPUS_VERSION` now stamps the retrieval corpus, not a hand-written chunk count: it is bumped when the ingested CPG manifest scope changes, a document is added or removed, or a document's licence or profile scope changes (`backend/src/guidelines/documents.ts`). Stamped at `guideline-corpus-v5` since the curated-corpus retirement of 09/09/26 (§11, `docs/decisions.md` D-002).
...
| 22  | Should any ASR post-correction exist beyond the deterministic mishear table? **Built 09/09/26 and shipped off; whether it earns its place is still open, and is now measurable.**                                                                   | §20.7, §20.9, §20.10 | **Built as doctor-accepted proposals, gated on recogniser confidence, off by default** (#309, §20.10). The shape shipped is not quite the one decided on 06/09/26: the replacement is bounded by a character class and a word-delta rather than drawn from a vocabulary enum, an owner decision of 09/09/26 recorded with its cost in §20.10 rather than left as drift. **The bar this row set is unchanged and still unmet**: the free deterministic table in `redflags/mishears.ts` is what the pass has to beat, and nothing has measured it. §20.8 records that no vendor documents a post-correction stage and that the technique hurts in the low-error regime; §20.9 shows the one form with a large reported effect needs an N-best list no provider here returns. `TRANSCRIPT_CLEANUP` stays `off` and `evals/transcript-cleanup.ts` is the harness that would settle it                                                                                                                                                                                                                    | Human (turn it on only against a number)                            |
| 23  | May MOH-ARR spans (the NAG document) be sent to the model for grounding at all, distinct from whether they may be displayed? Opened 09/09/26 with the curated-corpus retirement (`docs/decisions.md` D-002).                                        | §11                  | An explicit licence or operator decision on model-side use of all-rights-reserved text, separate from the display question §11's Licence Status already answers. **Current posture, stated rather than resolved:** MOH-ARR spans are sent to the model, de-identified like every other retrieved span, and display is off (`verbatimAllowed: false` renders attribution only, never the summary or quote).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Unassigned (issue #250)                                             |
...
- **Only when the row opens empty.** A create carrying a transcript is a consultation already captured some other way, and the mode locks the moment a transcript exists, so it is written `manual` exactly as it was before 21/09/26. An unconditional write would stamp every pasted, uploaded and fixture-seeded consultation with a claim that audio streamed to the provider, permanently and with no way to correct it.
...
#### Extended 10/09/26: The Sign-Off Reaches Prescription Dictation

The grant above is scoped to ambient capture, and its "What it does not" row names the manual record-then-transcribe path. Streaming a prescription the doctor dictates on the review page is neither of those, so it takes its own row rather than being read into the first one.

| Question         | Answer                                                                                                                                                                                  |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Who              | @Andersonnn7788, the repo owner, on 10/09/26                                                                                                                                            |
| What it covers   | Prescription dictation on the review page, on the same Soniox socket, browser-direct, US region                                                                                         |
| What it does not | Any other dictation surface, the note editor and the transcript-corrections box included; any third audio egress; any other transport                                                   |
| What prompted it | A reasoned extension, not a measurement. It adds no new counterparty, destination, transport or minting route, and one dictated phrase is a narrower exposure than a whole consultation |
...
#### Recorded 10/09/26: The Drug Half Was A Second Hand-Written Copy, And The Budget Is Now Full

`ENGLISH_DRUG_TERMS` was written by hand, while `backend/src/medications/lexicon.ts` separately holds the names the doctor can accept as a candidate. The two had drifted. Eleven lexicon drugs were primed nowhere, the entire urinary antibacterial set among them: clarithromycin, doxycycline, phenoxymethylpenicillin, nitrofurantoin, trimethoprim, cotrimoxazole, fosfomycin, ciprofloxacin, norfloxacin, cefalexin and phenazopyridine.

**The two halves of the chain were aiming at different targets**, which is the failure the vocabulary module's own header warns about when it derives `CONFUSABLE_TARGETS` from the `mishears.ts` map rather than copying it: a name the recogniser was never primed for is one `matchMedication` can only offer after the doctor has already typed it. #356 derives the drug terms from the lexicon, in the same shape and for the same reason.

**The cost is the remaining budget, all of it.** Measured 10/09/26, the deduped ambient list lands at **exactly 120 against `MAX_ASR_CONTEXT_TERMS = 120`**:

| Source                                     | Terms   |
| ------------------------------------------ | ------- |
| Malay consultation register                | 61      |
| Existing drug and dosage-form terms        | 28      |
| `MEDICATION_LEXICON` generics and synonyms | 39      |
| `CONFUSABLE_TARGETS`                       | 7       |
| **Deduped total**                          | **120** |

Nothing has to be dropped, and the brand names survive, which matters because a patient says "Panadol" where the lexicon deliberately holds only generics. What it means is that the next lexicon entry is not a free addition: it forces a decision about the cap. That cap is small on purpose. A list crossing the audio egress cannot be de-identified, so review by eye is the only review it can get, and 120 terms is already near the limit of what an eye will actually check.

**Prescription dictation sends a different, narrower list** (#356). The Malay symptom register and the confusable targets are excluded, because a prescription phrase carries no symptoms and 61 terms of the wrong domain is the effect §20.7.1 measured scoring worse than no context at all. What it sends instead is the lexicon plus the sig vocabulary the parser already owns, so every primed phrase is one `parseSig` can act on.
...
#### Built 09/09/26: Layer 3, The Constrained Cleanup Pass (Issue #309)

The third accuracy layer, built and shipped **off**. Layer 1 primes the recogniser, layer 2 is the measured 11-entry table, and this reaches the mishears neither can: §20.7.1 measured "batuk" returning as "betul", a pair no confusable table can claim without raising a cough flag on every sentence agreeing with the doctor.

`backend/src/transcript-cleanup/` holds it, reached from the existing `POST /api/consultations/:id/transcript-corrections` rather than a second route. It goes deid then `LLMClient` exactly as `draft_turns` does, and chunks after the gate.
...
§20.9's chosen shape says the replacement must be "drawn from a static, versioned clinical vocabulary, so an arbitrary word cannot be proposed. Same mechanism as ID-constrained citations". **What shipped is not that.** The owner chose the character-class shape on 09/09/26, after the alternative was put with its trade-off.

| §20.9 as decided 06/09/26                                  | Shipped 09/09/26                                                                          |
| ---------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Replacement is an enum over a versioned vocabulary         | Replacement matches a character class, at most two words                                  |
| Hallucinated or out-of-domain word structurally impossible | Hallucinated word bounded by the uncertainty gate and the two overlap rules, not excluded |
| Cannot fix a mishear whose target is outside the register  | Can fix any word-level mishear                                                            |

**State the cost plainly.** §20.7 names "no chest pain" becoming "some chest pain" as the harm this layer must not create, and §20.9 credited the vocabulary constraint with making it structurally unreachable. Under the shipped shape it is not unreachable: a negation flip is one word, passes the character class, and would be caught only if the span sits outside every uncertain range or overlaps a fired flag's evidence. The prompt forbids it, and a prompt is a Tier-4 control that does not count. `ASR_VOCABULARY_VERSION` already exists in `backend/src/lib/asr/vocabulary.ts`, so tightening this to an enum later is a small diff, and it is the first thing to reach for if a measurement shows over-correction.

##### Off By Default, And What Would Turn It On

§20.9's bar is unchanged and unmet: "the model pass earns its place only if it finds correct corrections the table does not. If it finds none, ship the deterministic proposals alone and record the model pass as measured and rejected."

`TRANSCRIPT_CLEANUP` defaults to `off` in `backend/src/config/env.ts` and is pinned `off` in `render.yaml`, so turning it on is a diff rather than a dashboard click. `evals/transcript-cleanup.ts` is the harness that produces the number: it reports corrections found beyond the table alongside over-corrections on already-correct text, because only the pair is meaningful. **Nothing in this repo has run it against a provider yet**, so no claim is made here about whether this layer works.

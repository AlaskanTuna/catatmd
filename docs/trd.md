...
| 19  | Should `note_and_gaps` be split into two operations, one for the clinical facts and one for the SOAP note?                                                                       | §12     | Opened 13/08/26 by the row 8 measurement. The fixed 29-key checklist is what makes "never silently omitted" enforceable (`docs/prd.md` §10), and it is also what makes every response large enough to sit near the provider's reliable output ceiling. Splitting the operation and running the two concurrently would roughly halve each response while keeping the key set intact, at the cost of a second call and a change to a Final §12 contract. The alternative is accepting a ~17% retry rate on the primary operation.                                                                                                                                                                                                                                                                                                                 | Human (architecture call)                                |

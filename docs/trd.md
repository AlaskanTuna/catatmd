...
| Security headers   | `Built`     | `helmet()` at `app.ts:31`, CSP left at its default posture because the SPA is served by Vercel, not by this API. **Gap:** `vercel.json` has no `headers` block, so the SPA origin itself sends no CSP, HSTS, X-Frame-Options or Referrer-Policy                                                                                                                                                                                                                                                                                                           |

...
Every row carries `actorId` (the authenticated doctor); every consultation-scoped row also carries `consultationId` (both columns already `Built`, §4), while the two `asr.*` rows are actor-only because they fire before any consultation exists. Together the taxonomy covers every transition in `docs/prd.md` §8 (Primary Flow): create → analyse (start/complete/fail) → edit/acknowledge → approve, plus the audio egress around it.

...
Every `Open` item in this document, collected in one place. **Resolved rows are struck through and kept, never deleted** — renumbering would break the many `§19, row N` cross-references in both documents. Ten rows closed on 13/08/26 (1, 2, 3, 4, 5, 6, 9, 10, 13, 14); four opened (15, 16, 17, 18). Rows 8 and 19 opened and closed the same day, by measurement. Rows 1, 2, 4 and 9 closed during implementation — each was assigned to the implementation task that reached it. Row 18 closed on 16/08/26 with the consent gate in §20.4. Rows 21 and 22 opened 05/09/26 with the ambient-capture research in §20.7.
...
| 21  | Which provider carries live multilingual capture, and is Tamil transcribable at all under the residency posture?                                                                                                                                    | §20.7   | The per-language measurement table in §20.7, run on synthetic clips. `qwen3-asr-flash` is the recommendation on §20.1 evidence, but that was one casual Manglish sample and covers neither Tamil, Mandarin nor Cantonese. If no in-region provider transcribes Tamil, the answer is a stated limitation, not a second vendor adopted speculatively                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Unassigned (issue #218)                                             |
| 22  | Should any ASR post-correction exist beyond the deterministic mishear table?                                                                                                                                                                        | §20.7   | Measured error rate per language once layers 1 to 3 of §20.7 are in place. A transcript-rewriting model is rejected outright there and is not what would unblock this; the only admissible answer is a doctor-accepted suggestion in the shape issue #195 shipped, so the question is whether the residual error rate justifies rebuilding it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Unassigned                                                          |
...
| Batch transcription, unchanged     | **`ilmu-asr-v4.2`** (ILMU, Malaysia)                    | Provider, via our relay           | Audio to text                                                                      | Change at all; this path is untouched by ambient work                                           |
...
| `ilmu-asr-v4.2`, ILMU                                                    | Malay and English only                                                                                                                                  | No, batch only                                       | Malaysia, the strongest residency answer available                  | Keep unchanged as the in-country batch option                                                                        |

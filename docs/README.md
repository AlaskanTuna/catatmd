...
**A recording now comes back as timestamped, speaker-labelled draft lines the doctor reviews and applies.** Each line's Doctor/Patient label is a guess from segment timing and question-mark punctuation, never from the voices, and the doctor can flip any line before applying, one review list, no auto-populated transcript (`docs/trd.md` §20.2).

...
export function buildSuggestionsSystemPrompt(
  ...
  const noCitableCorpus = corpus.length === 0

  const taskOne = noCitableCorpus
    ? `Task 1 - guideline-cited suggestions:
No citable guideline corpus is available for this consultation. Do not propose any guideline-cited suggestions, do not include any citations, and never state a diagnosis. Return an empty suggestions array.`
    : `Task 1 - guideline-cited suggestions:
...
guideline-aligned consideration for the doctor to weigh, not a conclusion.`
  ...
  const scope = noCitableCorpus
    ? `Scope:
No citable guideline corpus is available. Set outOfScope to true and return an
empty suggestions array. Still report any red-flag candidates regardless of scope.`
    : `Scope:
...
add.`
  ...
  const corpusBlock = noCitableCorpus
    ? `Guideline corpus:
No citable guideline corpus is available for this consultation.`

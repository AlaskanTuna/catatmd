...
const MEDICATION_ACTION_ORDER = new RegExp(
  `\\b(?:start|give|administer|dispense)\\b[\\s\\S]{0,40}\\b${MEDICATION_TERM}\\b`,
  'i',
)
...
const BARE_DOSE_REGIMEN = new RegExp(
  `^\\s*(?:${MEDICATION_TERM}\\s+)?\\d+(?:\\.\\d+)?\\s*(?:mg|g|mcg|μg|ug|ml|units?|iu)\\b(?:\\s*(?:/|per)\\s*(?:kg|day|dose)|[\\s,]*(?:once|twice|thrice|daily|every\\s+\\d+\\s*(?:hours?|h)|(?:one|two|three|four)\\s+times?(?:\\s+daily)?|(?:q|od|bd|bid|tds|tid|qid|qhs|qds|stat)\\b)|[\\s,]*for\\s+\\d+\\s+days?)?`,
  'i',
)
...
function containsUnsafeMedicationProse(text: string): boolean {
  return text.split(/(?<=[.!?])\s+/).some((sentence) => {
    ...
    if (SAFE_MEDICATION_CONTEXT.test(candidate)) return false

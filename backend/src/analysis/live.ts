 ...
 */
function fold(previous: unknown, incoming: unknown): unknown {
  if (isAssertion(previous) && isAssertion(incoming)) {
    return established(previous) && !established(incoming) ? previous : incoming
  }
  if (Array.isArray(previous) && Array.isArray(incoming)) {
    // `medicationsDispensed`. A window that dispensed nothing says nothing
    // about the window that listed something, so an empty list never erases.
  ...
  if (isRecord(previous) && isRecord(incoming)) {
    ...
    for (const [key, before] of Object.entries(previous)) {
      merged[key] = incoming[key] === undefined ? before : fold(before, incoming[key])
    }
    return merged
  }
  return incoming
...
export function foldFacts<T>(previous: T | null | undefined, incoming: T): T {
  ...
  return fold(previous, incoming) as T
}

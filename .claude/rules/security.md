...
The consent rule is identical on all of them and is two controls, needing both: a standing device preference in `localStorage` (the engine for the relay, the capture mode for ambient, `dictationEngine` for prescription dictation) and a per-consultation tick in plain `useState` that dies with the component (#155, restored by #254 after #228 briefly collapsed the two into one remembered setting). It is client-side only, because the API still enforces no consent signal. On the relay the tick is enforced in `AudioCapture`'s transcribe dispatcher rather than only on the controls, because the engine can change mid-recording and a disabled button cannot catch that; on ambient in `AmbientCapture`'s start dispatcher, and on prescription dictation in the review page's, for the same reason. Never fold the tick back into the preference, and never let any of the three dispatchers fall through to another path instead of refusing.

**On prescription dictation, one of those two controls now ships pre-set, and the count of live controls there is one.** `DEFAULT_AUDIO_SETTINGS.dictationEngine` became `'streaming'` on 10/09/26 (@Andersonnn7788, `docs/decisions.md` D-001, #363). A preference the doctor has not moved is not a choice they made, so that surface is a defaulted preference plus a deliberate tick rather than two decisions. Do not describe it as two. What did not change, and must not: the tick is untouched, is still asked once per consultation, is still remembered by nothing, and is still the only thing that opens the socket. The loader keeps the weakening from spreading, and the split is deliberate: an **absent** `dictationEngine` reads as the current default, while any **present** value other than `'streaming'` reads as `'local'`, so corruption still fails closed. The relay's `engine` and ambient's capture mode are unchanged and still default to the value that sends nothing.

**The review page is the third place this gate lives, and #313 avoided building it there deliberately before #355 decided otherwise.** Three things are specific to it.

**The standing half was unreachable from that page until #363, which is worth remembering before trusting a control to exist because a rule names it.** `AudioSettingsDialog` is the single writer of the preference and was mounted only in `CapturePanel`, which renders only while there is no transcript, while the card the preference governs needs one. The switch and the microphone it governed could never be on screen together, so for the whole of #357's life the two-control rule was two controls on paper and one in the interface. The dialog is now mounted on both surfaces; they stay mutually exclusive by those same render conditions, which is what keeps it a single writer rather than two screens racing to overwrite one stored object. Falling through to the on-device worker when consent is withheld is forbidden in the same way falling through to the cloud is: it gives a different result with no word that it happened, so a refusal stays a refusal and an on-device run takes a second deliberate press. And the tick is per consultation rather than per drug, because one review page can carry several prescriptions on one patient: the component stays mounted across them, so clearing the tick between them would ask the same patient once per drug.

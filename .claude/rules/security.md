...
- **An edit whose span overlaps the evidence of a fired rule flag is dropped when its `source` is `model`, and is not when its `source` is `mishear`.** `evaluateRedFlags` runs in-process over the raw transcript before any egress, for exactly this. A flag whose evidence cannot be located in any turn protects the whole transcript. This is what keeps AGENTS.md's zero-suppression rule from being one doctor's click away; `mergeRedFlags` itself is untouched.

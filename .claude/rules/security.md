...
- Cookies are `httpOnly`, `secure`, and `sameSite: 'none'` in production. The cost is that `SameSite` provides no CSRF protection here, so origin trust rests entirely on the single-origin CORS policy plus `trustedOrigins`. Widening `CORS_ORIGIN` to a list, a wildcard, or a reflected origin removes the only remaining control.

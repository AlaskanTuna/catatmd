...
- **Deployment is not symmetric between collaborators.** A merge to `main` runs `verify` for everyone, but **shipping the result is currently tied to @AlaskanTuna's Vercel and Render accounts**. Observed rather than theorised: a push from the other collaborator did not reach production, and the same asymmetry appeared earlier as `TEAM_ACCESS_REQUIRED` (13/08/26, `c2321c4`), because the Vercel project is on a Hobby plan that only builds commits authored by the account owner.
  - `.github/workflows/ci.yml` carries a mitigation for that author check, and its comment claims collaborator commits now deploy on their own merge. **That claim is not currently borne out.** Treat it as unverified, not as the state of the world.

...
- **Two lanes to `main`, chosen by what the change is, not by how big it is.**
  ...
  - **A stacked PR is the one case where `--delete-branch` is wrong.** Deleting a branch closes every PR still based on it rather than retargeting them, so the child is lost at the moment the parent lands. Observed 09/09/26: #351 was merged with `--delete-branch`, and #352, based on it, was closed with its base still pointing at the deleted branch; recovering it meant cherry-picking the same commit onto a fresh branch off `main` as #353. Either merge the child first, or merge the parent without `--delete-branch` and sweep the branch once the child has landed.
...
- **Deployment is not symmetric between collaborators.** A merge to `main` runs `verify` for everyone, but **shipping the result is currently tied to @AlaskanTuna's Vercel and Render accounts**. Observed rather than theorised: a push from the other collaborator did not reach production, and the same asymmetry appeared earlier as `TEAM_ACCESS_REQUIRED` (13/08/26, `c2321c4`), because the Vercel project is on a Hobby plan that only builds commits authored by the account owner.
  - `.github/workflows/ci.yml` carries a mitigation for that author check, and its comment claims collaborator commits now deploy on their own merge. **One verified counter-example now exists, 06/09/26:** PR #261 squash-merged as `695c3a5`, authored by @Andersonnn7788, and `Deploy to Vercel` reported `success` on run `34021139348`, with that run's own `Assert /api reaches the API through the rewrite` step passing against the deployment. So the mitigation does work at least once, for a diff touching `frontend/`.
  - **That is one data point, not a general guarantee.** It says nothing about Render, about a backend-only diff, or about whether the earlier failures had a different cause. Until it has held a few more times, still check rather than assume.
  - **If you are not @AlaskanTuna, do not assume your merge deployed.** Check `gh run list --branch main` after any merge you expect to ship, or ask him to re-trigger. A skipped deploy is silent, and the failure mode is production quietly sitting behind `main`.
  - A docs-only merge is _expected_ not to deploy, because `docs/**` sits outside the deployable-paths filter (#82, #83, #84). Do not mistake that for the asymmetry above.
